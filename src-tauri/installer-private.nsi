; Derived from Tauri CLI 2.10.1 installer.nsi (MIT OR Apache-2.0).
; Source: https://github.com/tauri-apps/tauri/blob/tauri-cli-v2.10.1/crates/tauri-bundler/src/bundle/windows/nsis/installer.nsi
; Private changes: isolated keys/paths; no upstream WiX detection, file associations,
...
!define UNINSTKEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRIVATE_INSTALL_NAME}"
!define MANUKEY "Software\${MANUFACTURER}"
!define MANUPRODUCTKEY "${MANUKEY}\${PRIVATE_INSTALL_NAME}"
...
!define PLACEHOLDER_INSTALL_DIR "placeholder\${PRIVATE_INSTALL_NAME}"
...
!if "${INSTALLMODE}" == "both"
  ...
  !define MULTIUSER_INSTALLMODE_INSTDIR "${PRIVATE_INSTALL_NAME}"
...
Function PageReinstall
  ; Never adopt an upstream installation based on its visible product name.
  StrCpy $WixMode 0
...
Function .onInit
  ...
  ${If} $INSTDIR == "${PLACEHOLDER_INSTALL_DIR}"
    ...
    !if "${INSTALLMODE}" == "perMachine"
      ${If} ${RunningX64}
        !if "${ARCH}" == "x64"
          StrCpy $INSTDIR "$PROGRAMFILES64\${PRIVATE_INSTALL_NAME}"
        !else if "${ARCH}" == "arm64"
          StrCpy $INSTDIR "$PROGRAMFILES64\${PRIVATE_INSTALL_NAME}"
        !else
          StrCpy $INSTDIR "$PROGRAMFILES\${PRIVATE_INSTALL_NAME}"
        !endif
      ${Else}
        StrCpy $INSTDIR "$PROGRAMFILES\${PRIVATE_INSTALL_NAME}"
      ${EndIf}
...
Section Install
  ...
  ; Private fork does not change global file associations.
...
Function CreateOrUpdateStartMenuShortcut
  ; A user-selected Start Menu folder may already contain the upstream link.
  ; Existing private links also remain valid because the binary name is stable.
...
Function CreateOrUpdateDesktopShortcut
  ; Preserve any existing shortcut (it may belong to the upstream release).
